...
class CpxMcaDecoder : public McaDecoder
{
  ...
  protected:
    ...
    const std::map<uint16_t, const char*> Mscod3122 = {
    ...
    const std::map<uint8_t, const char*> Mscod2116 = {
        {0x0, "UC Phy Initialization Failure"},
        {0x1, "UC Phy Detected Drift Buffer Alarm"},
        {0x2, "UC Phy Detected Latency Buffer Rollover"},
        {0x10, "UC LL Rx detected CRC error"},
...
class CpxMcaBank4 final : public CpxMcaDecoder
{
  private:
    union MC_STATUS
    {
        struct
        {
            uint64_t mcacod : 16, mscod_19_16 : 4, mscod_23_20 : 4,
...
class CpxMcaBank5 final : public CpxMcaDecoder
{
  private:
    union MC_STATUS
    {
        struct
        {
            uint64_t mcacod : 16, mscod_21_16 : 6, mscod_31_22 : 10,
    ...
    void decodeKnownFields()
    {
        ...
        std::stringstream ss;
        auto mscod2116Decoded = getDecoded(
            Mscod2116, static_cast<uint8_t>(status_decoded.mscod_21_16));
        if (mscod2116Decoded)
        {
            entry["MSCOD_21_16_decoded"] = *mscod2116Decoded;
            ss << *mscod2116Decoded << " | ";
        }

        auto mscod3122Decoded = getDecoded(
            Mscod3122, static_cast<uint16_t>(status_decoded.mscod_31_22));
        if (mscod3122Decoded)
        {
            entry["MSCOD_31_22_decoded"] = *mscod3122Decoded;
            ss << *mscod3122Decoded;
        }
  ...
  public:
    ...
    json decode_status() override
    {
        entry["MCACOD"] =
            int_to_hex(static_cast<uint16_t>(status_decoded.mcacod));
        entry["MSCOD"] =
            int_to_hex(static_cast<uint16_t>(status_decoded.mscod_21_16 |
                                            status_decoded.mscod_31_22 << 6));
        entry["MSCOD_21_16"] =
            int_to_hex(static_cast<uint8_t>(status_decoded.mscod_21_16));
        entry["MSCOD_31_22"] =
            int_to_hex(static_cast<uint16_t>(status_decoded.mscod_31_22));
        entry["OTHER_INFO"] =
...
class CpxMcaBank12 final : public CpxMcaDecoder
{
  private:
    ...
    void decodeKnownFields()
    {
        ...
        std::stringstream ss;
        auto mscod2116Decoded = getDecoded(Mscod2116,
            static_cast<uint8_t>(status_decoded.mscod_21_16));
        if (mscod2116Decoded)
        {
            entry["MSCOD_21_16_decoded"] = *mscod2116Decoded;
            ss << *mscod2116Decoded << " | ";
        ...
        auto mscod3122Decoded = getDecoded(Mscod3122,
            static_cast<uint16_t>(status_decoded.mscod_31_22));
        if (mscod3122Decoded)
        {
            entry["MSCOD_31_22_decoded"] = *mscod3122Decoded;
            ss << *mscod3122Decoded;
